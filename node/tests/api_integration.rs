...
async fn test_auth_create_proposal() {
    ...
    assert_eq!(
        resp.status(),
        201,
        "Valid JWT should yield 201 for proposal creation"
    );
    let body: Value = resp.json().await.unwrap();
...
async fn test_auth_get_balance() {
    ...
    let resp = client
        .get(format!(
            "{}/api/v1/economics/balance/did:test:unknown",
            server.base_url
        ))
        .send()
        .await
    ...
    for _ in 0..10 {
        ...
        if resp.status() == 429 {
            ...
            assert!(
                ...
                "429 response should contain 'error' field"
            );
...
async fn test_mint_ubc_admin_ok() {
    ...
    assert_eq!(
        resp.status(),
        200,
        "Admin caller should get 200 OK for MintUbc"
    );
    let body: Value = resp.json().await.unwrap();
...
async fn test_advance_epoch_admin_ok() {
    ...
    assert_eq!(
        resp.status(),
        200,
        "Admin caller should get 200 OK for AdvanceEpoch"
    );
    let body: Value = resp.json().await.unwrap();
    assert_eq!(body["status"].as_str().unwrap(), "processed");
...
async fn test_error_format_404_not_found() {
    ...
    assert!(
        !error_msg.is_empty(),
        "404 error message should not be empty"
    );

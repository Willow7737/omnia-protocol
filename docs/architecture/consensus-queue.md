...
Without the queue, the consensus engine would need to scan the entire causal graph on each iteration to find unprocessed events. For a graph with N events, this would be O(N) per iteration, which is unacceptable for a protocol targeting 10,000 TPS (v0.1.48 target; v0.1.76+ measured: 12,000 ops/s).

...
| Throughput | ~100-1000 TPS | ~7,190 events/sec (single-node, synchronous) |
...
⚠️ The `CausalGraph` uses an `unprocessed_events` queue so that consensus only processes new events each round — O(new_events) processing. **Actual measured single-node throughput is ~7,190 events/sec** (synchronous, no async overhead; see [benchmark-gates.md](../reference/benchmark-gates.md) for full benchmark results). Earlier tokio-based measurements showed ~527 events/sec due to async runtime overhead, not a consensus bottleneck — switching to direct synchronous calls yielded a **13.6× improvement**. Multi-node distributed throughput will be lower due to network latency and BFT consensus requirements. CausalGraph insertion is O(1) amortized via hash map operations, not O(1) guaranteed.

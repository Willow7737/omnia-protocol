...
> The previous ~527 events/sec ceiling was caused by tokio async runtime overhead, not the consensus pipeline itself. Direct synchronous calls yield ~7,190 events/sec — a **13.6× improvement** that revealed the true pipeline throughput.
...
2. **True pipeline throughput is ~7,190 events/sec** — the earlier ~527 events/sec ceiling was a measurement artifact from tokio async runtime overhead, not a consensus bottleneck. Switching to direct synchronous calls yielded a 13.6× improvement that revealed the real throughput.

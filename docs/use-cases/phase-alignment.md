...
|-----------|---------------|-------|
| Causal graph consensus | ✅ | 454+ substrate tests, ~7,190 events/sec synchronous single-node |
...
Phase 5 captured real benchmark data (~7,190 events/sec synchronous single-node, 13.6× improvement over earlier tokio-based measurements), validated multi-node BFT consensus, migrated VRF to ECVRF per RFC 9381, added genesis tooling, and prepared the external audit package.

### What Changed for Users

- **All Users**: Aspirational throughput claims have been replaced with honest measured data: ~7,190 events/sec synchronous single-node (13.6× improvement over earlier tokio-based measurements which showed ~527 evt/s due to async runtime overhead). This transparency allows realistic planning.
...
| Capability | Implementation | Notes |
|-----------|---------------|-------|
| Real benchmarks | ✅ | ~7,190 evt/s sync single-node (13.6× over earlier async measurements) |

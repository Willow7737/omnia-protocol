...
- **Throughput**: 80% threshold allows for ~20% variance due to environmental factors while catching real regressions. The jump from earlier tokio-based measurements (~527 evt/s) to ~7,190 events/sec was a methodology fix (switching to direct synchronous calls), not a code improvement — the async overhead was masking true pipeline throughput.

    ...
    pub fn process_transition(
        ...
        match order.state {
            ...
            PaymentState::RefundPending => {
                self.release_reservation_for_refund(order)
            }
...
mod tests {
    ...
    fn advance_to_risk_approved(order_id: &str) -> PaymentEngine {
        ...
        engine
            .advance_state(order_id, PaymentState::Quoted, Caller::System { service: "quote-service".into() }, NOW + 1000, Some("quoted".into()))
            .expect("quote");
        engine
            .advance_state(order_id, PaymentState::PaymentPending, Caller::System { service: "payment-service".into() }, NOW + 2000, None)
            .expect("payment pending");
        engine
            .advance_state(order_id, PaymentState::PaymentVerified, Caller::Provider { provider_id: "MTN".into(), authenticated: true }, NOW + 3000, Some("provider-ref".into()))
            .expect("payment verified");
        engine
            .advance_state(order_id, PaymentState::RiskReview, Caller::System { service: "risk-engine".into() }, NOW + 4000, None)
            .expect("risk review");
        engine
            .advance_state(order_id, PaymentState::RiskApproved, Caller::System { service: "risk-engine".into() }, NOW + 5000, None)
    ...
    fn reserve_inventory_on_approval() {
        ...
        engine
            .advance_state("order-reserve-test", PaymentState::InventoryReserved, Caller::Treasury, NOW + 6000, None)
    ...
    fn release_on_refund() {
        ...
        // Reserve
        engine
            .advance_state("order-refund-test", PaymentState::InventoryReserved, Caller::Treasury, NOW + 6000, None)
            .expect("reserve");
        {
        ...
        // Fail and refund
        engine
            .advance_state("order-refund-test", PaymentState::PaymentFailed, Caller::Provider { provider_id: "MTN".into(), authenticated: true }, NOW + 7000, Some("failed".into()))
            .expect("fail");
        engine
            .advance_state("order-refund-test", PaymentState::RefundPending, Caller::System { service: "refund-service".into() }, NOW + 8000, None)
    ...
    fn idempotent_reserve() {
        ...
        engine
            .advance_state("order-idem-test", PaymentState::InventoryReserved, Caller::Treasury, NOW + 6000, None)
            .expect("reserve");
        {
    ...
    fn consume_reservation_on_delivery() {
        ...
        engine.advance_state("order-deliver-test", PaymentState::InventoryReserved, Caller::Treasury, NOW + 6000, None).expect("reserve");
        {
            let order = engine.get_order_mut("order-deliver-test").expect("exists");
            adapter.process_transition(order, PaymentState::RiskApproved, NOW + 6000).expect("adapter reserve");
        }
        engine.advance_state("order-deliver-test", PaymentState::AllocationSubmitted, Caller::System { service: "chain-service".into() }, NOW + 7000, None).expect("alloc submitted");
        engine.advance_state("order-deliver-test", PaymentState::AllocationFinalized, Caller::System { service: "chain-service".into() }, NOW + 8000, None).expect("alloc finalized");
        engine.advance_state("order-deliver-test", PaymentState::Delivered, Caller::System { service: "delivery-service".into() }, NOW + 9000, None).expect("delivered");
    ...
    fn cancel_releases_reservation() {
        ...
        engine.advance_state("order-cancel-test", PaymentState::InventoryReserved, Caller::Treasury, NOW + 6000, None).expect("reserve");
        {
            let order = engine.get_order_mut("order-cancel-test").expect("exists");
            adapter.process_transition(order, PaymentState::RiskApproved, NOW + 6000).expect("adapter reserve");
        }

        engine.advance_state("order-cancel-test", PaymentState::Cancelled, Caller::Sender, NOW + 7000, Some("user cancelled".into())).expect("cancel");
    ...
    fn full_happy_path_with_treasury() {
        ...
        // Reserve
        engine.advance_state("order-happy-full", PaymentState::InventoryReserved, Caller::Treasury, NOW + 6000, None).expect("reserve");
        {
            let order = engine.get_order_mut("order-happy-full").expect("exists");
            let res = adapter.process_transition(order, PaymentState::RiskApproved, NOW + 6000).expect("adapter reserve");
            assert!(matches!(res, TreasuryBridgeResult::Reserved { .. }));
        }

        // Advance through allocation
        engine.advance_state("order-happy-full", PaymentState::AllocationSubmitted, Caller::System { service: "chain-service".into() }, NOW + 7000, None).expect("alloc submitted");
        engine.advance_state("order-happy-full", PaymentState::AllocationFinalized, Caller::System { service: "chain-service".into() }, NOW + 8000, None).expect("alloc finalized");
        engine.advance_state("order-happy-full", PaymentState::Delivered, Caller::System { service: "delivery-service".into() }, NOW + 9000, None).expect("delivered");

        // Consume
        {
            let order = engine.get_order_mut("order-happy-full").expect("exists");
            let res = adapter.process_transition(order, PaymentState::AllocationFinalized, NOW + 9000).expect("consume");
